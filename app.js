...
(function () {
	...
	try {
		global.BotConfig = JSON.parse(fs.readFileSync('./config/RebootFN/config.json', 'utf8', function (err, data) {
	...
	if (!BotConfig) {
		fs.mkdirSync(`./config/RebootFN/`, { recursive: true });
		fs.writeFile(`./config/RebootFN/config.json`, JSON.stringify({
	...
	app.listen(port, () => {
		console.log(`LvFn v${version} Cumming on port ${port}!`);
	});
